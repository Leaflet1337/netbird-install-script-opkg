Техническая инструкция по массовому развертыванию NetBird на роутерах Keenetic (Entware)
Эта техническая инструкция предназначена для системных администраторов для массового развертывания клиента NetBird на роутерах Keenetic (архитектура MIPS/MIPSLE/AARCH64, среда Entware).
...
Установлена и настроена среда Entware (на USB-флешку или во внутреннюю память роутера).

В веб-интерфейсе Кинетика включен компонент «WireGuard VPN» (необходим для инициализации модуля tun в ядре).
...
Статус подключения клиента к сети:
...
Откройте Web-интерфейс роутера -> Сетевые правила -> Межсетевой экран.

Вкладка Правила для сегментов -> Выберите сегмент «Домашняя сеть» (или тот сегмент, куда настраивается доступ).
